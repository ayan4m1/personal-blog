...
const fireMaterial = {
  ...
  uniforms: {
    ...
    magnitude: { type: 'f', value: 0.5 },
    lacunarity: { type: 'f', value: 4.0 },
...
export default function FireCone(props) {
  const ref = useRef();
  const texture = useLoader(TextureLoader, '/assets/fire.png');
  ...
  useLayoutEffect(() => {
    ...
    ref.current.material.uniforms.color.value = new Color(0xeeeeee);
    ref.current.material.uniforms.invModelMatrix.value = new Matrix4();
    ref.current.material.uniforms.scale.value = new Vector3(1, 1, 1);
  ...
  return (
    ...
    >
      ...
      <shaderMaterial {...fireMaterial} transparent />
    </mesh>
  );
}
